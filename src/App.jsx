...
function CustomCursor() {
  ...
  useEffect(() => {
    ...
    const onOver = e => { if (e.target.matches('button,a,[role="button"],.pillar,.book-card,.sdot,.disciple-card,.miracle-card,.correct-card')) ringRef.current?.classList.add('hovering') }
...
function ScrollProgress() {
  ...
  useEffect(()=>{
    const fn=()=>{const el=document.documentElement;setPct(el.scrollTop/(el.scrollHeight-el.clientHeight)*100||0)}
...
export default function App() {
  ...
  const navigateTo = useCallback(target=>{
    ...
    setTrans(true)
    setTimeout(()=>{setPage(target);window.scrollTo(0,0);setScrollY(0);setTrans(false)},340)
  },[page])
    ...
    <div className="app">
      ...
      <div className={`page-wrap ${trans?'out':'in'}`}>
        <header>
          ...
          >
            <KopeckySymbol size={68} glow={true}/>
          </div>
